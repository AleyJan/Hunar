...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      // Save to AsyncStorage individually for Web Bundler Compatibility
      await AsyncStorage.setItem(STORAGE_KEY_USER_ID, user.userId);
      await AsyncStorage.setItem(STORAGE_KEY_TOKEN, user.token);
      await AsyncStorage.setItem(STORAGE_KEY_NAME, user.name || "User");
  ...
  const signup = async (name, email, password) => {
    try {
      ...
      await AsyncStorage.setItem(STORAGE_KEY_USER_ID, user.userId);
      await AsyncStorage.setItem(STORAGE_KEY_TOKEN, user.token);
      await AsyncStorage.setItem(STORAGE_KEY_NAME, user.name || name);
