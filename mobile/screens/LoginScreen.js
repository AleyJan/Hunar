...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (!email || !password) {
      Alert.alert("Error", "Email aur password dono daalein");
      return;
    }
    if (!email.includes('@')) {
      Alert.alert("Error", "Valid email daalein");
      return;
    }
    ...
    try {
      ...
      // On success: navigation handles redirect automatically via AuthContext state change
    } catch (error) {
      Alert.alert("Login Failed", error.message || "Apka login fail ho gaya.");
    } finally {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Login to Hunar</Text>
      ...
      <TouchableOpacity onPress={() => navigation.navigate(SCREENS.SIGNUP)} style={styles.linkButton}>
